...
def processar_base_sipcv(input_path: str, output_path: str):
    """
    Realiza a leitura, limpeza e filtragem da base SIPCV com foco no
    Spatial RDD para analisar a infraestrutura das DDMs.
    """
    ...
    if 'RUBRICA' in df.columns:
        mask_crime = mask_crime | df['RUBRICA'].fillna('').str.upper().str.contains('|'.join(termos_crime), regex=True)
    if 'NATUREZA APURADA' in df.columns:
        mask_crime = mask_crime | df['NATUREZA APURADA'].fillna('').str.upper().str.contains('|'.join(termos_crime), regex=True)
    ...
    termos_violencia = ['DOM.STICA', 'DOMESTICA', 'MARIA DA PENHA', 'FAMILIAR', 'AFETIVA']
    ...
    for col in colunas_contexto:
        if col in df.columns:
            mask_violencia = mask_violencia | df[col].fillna('').str.upper().str.contains('|'.join(termos_violencia), regex=True)
    ...
    if mask_violencia.sum() > 10:
        ...
        print(f"Aviso: Filtro restrito achou apenas {mask_violencia.sum()} casos explícitos. Flexibilizando e mantendo a amostra de agressão contra a mulher ({len(df)} registros).")
    ...
    colunas_desejadas = [
        ...
        'RUBRICA', 'NATUREZA APURADA', 'CONTEXTO', 'FLAG_INTOLERANCIA', 'TIPO_INTOLERANCIA',
        # Georreferenciamento (Running Variable - Distância)
        'LATITUDE', 'LONGITUDE', 'LOGRADOURO', 'NUMERO_LOGRADOURO', 'CIDADE',
        # Jurisdição (Tratamento - D)
        'NOME_DELEGACIA_CIRC', 'NOME_SECCIONAL_CIRC', 'NOME_DELEGACIA', 'NOME_DEPARTAMENTO_CIRC',
        # Controles Sociais (Covariáveis - X)
        'SEXO_PESSOA', 'IDENTIDADE_GENERO', 'IDADE_DATA_OCORRENCIA', 'COR_CURTIS', 'NOME_PROFISSAO'
    ]
